...
def _append_excel_row(file_name, columns, values):
    if os.path.isfile(file_name):
        records_df = pd.read_excel(file_name)
    else:
        records_df = pd.DataFrame(columns=columns)

    # Assigning the row directly avoids pandas' deprecated empty/all-NA concat behavior.
    records_df.loc[len(records_df)] = values
    parent_dir = os.path.dirname(file_name)
    if parent_dir:
        os.makedirs(parent_dir, exist_ok=True)
    records_df.to_excel(file_name, index=False)
...
class TradeRecord:
    ...
    def write_to_excel(self, file_name="res/trades.xlsx"):
        ...
        values = [self.date, self.session, self.stock_type, self.buyer, self.seller, self.quantity, self.price]
        _append_excel_row(file_name, columns, values)
...
class StockRecord:
    ...
    def write_to_excel(self, file_name="res/stocks.xlsx"):
        ...
        values = [self.date, self.session, self.stock_a_price, self.stock_b_price]
        _append_excel_row(file_name, columns, values)
...
class AgentRecordDaily:
    ...
    def write_to_excel(self, file_name="res/agent_day_record.xlsx"):
        columns = ["交易员", "交易日", "是否贷款", "贷款类型", "贷款数量",
                   "明日是否贷款", "明日是否买入A", "明日是否卖出A", "明日是否买入B", "明日是否卖出B"]
        values = [self.agent, self.date, self.if_loan, self.loan_type, self.loan_amount,
...
class AgentRecordSession:
    ...
    def write_to_excel(self, file_name="res/agent_session_record.xlsx"):
        columns = ["交易员", "交易日", "交易阶段", "交易前资产总额",
                   "交易前持有现金", "交易前持有的A股价值", "交易前持有的B股价值",
                   "挂单类型", "挂单股票类别", "挂单数量", "挂单价格"]
        values = [self.agent, self.date, self.session, self.proper, self.cash,
